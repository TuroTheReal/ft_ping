...
int receive_ping(t_ping *ping, t_stats *stats) {
	...
	// Utiliser timeout seulement si défini, sinon attendre indéfiniment
	double wait_time = (ping->timeout > 0) ? ping->timeout : ping->interval;
	timeout.tv_sec = (int)wait_time;
	timeout.tv_usec = (int)((wait_time - (int)wait_time) * 1000000);
...
void do_ping(t_ping *ping, t_stats *stats) {
	...
	// Ne pas afficher \n ici, sera fait par signal_handler ou après
	if (ping->running) {
		printf("\n");
	}
}
